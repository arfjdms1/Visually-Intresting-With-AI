...
def slugify(text: str) -> str:
    ...
    text = re.sub(r'-+', '-', text); return text.strip('-')
...
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None):
    ...
    return jwt.encode(to_encode, SECRET_KEY, algorithm=ALGORITHM) # This line will now work
...
async def get_leaderboard():
    ...
    all_models = [ {**model, 'company': company_data.get("name")} for company_data in db.values() for model in company_data.get("models", []) ]
    return sorted(all_models, key=lambda x: (-x.get("votes", 0), x.get("name")))[:5]

@app.post("/api/models/{model_id}/vote", status_code=200)
async def vote_for_model(model_id: int, request: Request):
    client_ip = request.client.host; votes_db = await read_data(VOTE_BIN_ID)
    voters_for_model = votes_db.get(str(model_id), [])
    if client_ip in voters_for_model: raise HTTPException(status_code=403, detail="You have already voted for this model.")
    db = await read_data(DB_BIN_ID); model_to_update = None
    for company_data in db.values():
        for model in company_data.get("models", []):
            if model.get("id") == model_id: model["votes"] = model.get("votes", 0) + 1; model_to_update = model; break
        if model_to_update: break
    if not model_to_update: raise HTTPException(status_code=404, detail="Model not found.")
    voters_for_model.append(client_ip); votes_db[str(model_id)] = voters_for_model
    await write_data(votes_db, VOTE_BIN_ID); await write_data(db, DB_BIN_ID)
...
async def create_company(request: Request, current_user: dict = Depends(get_current_user)):
    body = await request.json(); company_name = body.get("companyName")
...
async def add_model_to_company(company_slug: str, model_in: ModelIn, current_user: dict = Depends(get_current_user)):
    ...
    new_model = { "id": int(datetime.now().timestamp() * 1000), "name": model_in.name, "description": model_in.description, "htmlContent": model_in.htmlContent, "votes": 0 }
...
async def delete_model(company_slug: str, model_id: int, current_user: dict = Depends(get_current_user)):
    data = await read_data(DB_BIN_ID)
    if company_slug not in data: raise HTTPException(status_code=404, detail="Company not found.")
    models_list = data[company_slug].get("models", []); initial_length = len(models_list)
...
@app.get("/{path:path}")
async def serve_frontend(path: str): return FileResponse("static/index.html")