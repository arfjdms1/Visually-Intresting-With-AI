    ...
    const app = {
        ...
        attachEventListeners() {
            ...
            this.elements.adminContainer.addEventListener('click', (e) => {
                ...
                if (addBtn) this.addModel(addBtn.dataset.companySlug);
                if (delBtn) this.deleteModel(delBtn.dataset.companySlug, delBtn.dataset.modelId);
            });
            this.elements.searchInput.addEventListener('input', (e) => this.handleSearch(e.target.value));
            this.elements.publicContainer.addEventListener('click', (e) => {
                const voteBtn = e.target.closest('.vote-button');
                if (voteBtn) this.handleVote(voteBtn.dataset.modelId, voteBtn);
            });
        },
        ...
        async addModel(companySlug) {
            const name = document.getElementById(`model-name-${companySlug}`).value.trim();
            const description = document.getElementById(`model-desc-${companySlug}`).value.trim();
            const htmlContent = document.getElementById(`model-html-${companySlug}`).value.trim();
            if (!name || !htmlContent) return alert('Model name and HTML content are required.');
            await fetch(`/api/companies/${companySlug}/models`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...this.getAuthHeaders() }, body: JSON.stringify({ name, description, htmlContent }) });
            await this.switchView('admin');
        },
        async deleteModel(companySlug, modelId) {
            if (!confirm('Are you sure?')) return;
            await fetch(`/api/models/${companySlug}/${modelId}`, { method: 'DELETE', headers: this.getAuthHeaders() });
            await this.switchView('admin');
        },
        async renderLeaderboard() {
            try {
                ...
                this.elements.leaderboardList.innerHTML = '<li>Could not load leaderboard.</li>';
            }
        },
        handleHeaderScroll() {
        ...
        async handleVote(modelId, buttonElement) {
            ...
            try {
                ...
                if (!response.ok) throw new Error(result.detail || "Failed to vote");
                buttonElement.previousElementSibling.textContent = result.new_votes;
        ...
        renderAdminView(data) {
            ...
            for (const [slug, companyData] of Object.entries(data)) {
                const models = companyData.models || [];
                const modelListHTML = models.map(model => `<li><div class="model-info"><span class="model-name">${model.name}</span><p class="model-description">${model.description || ''}</p></div><button class="delete-model" data-company-slug="${slug}" data-model-id="${model.id}">&times;</button></li>`).join('');
                this.elements.adminContainer.innerHTML += `<div class="company-card"><h3>${companyData.name}</h3><div class="add-model-form"><input type="text" id="model-name-${slug}" placeholder="New Model Name" class="ui-input"><textarea id="model-desc-${slug}" placeholder="Brief model description..."></textarea><textarea id="model-html-${slug}" placeholder="Paste model HTML payload..."></textarea><button class="ui-button btn-gradient add-model-button" data-company-slug="${slug}"><span>Add Model</span></button></div><div class="model-list"><h4>Registered Models</h4><ul>${modelListHTML}</ul></div></div>`;
            }
        ...
        renderPublicView(data) {
            ...
            if (Object.keys(data).length === 0) {
                this.elements.publicContainer.innerHTML = '<div class="ui-panel" style="text-align: center; color: var(--text-secondary);">No models registered yet.</div>';
                return;
            }
            for (const [slug, companyData] of Object.entries(data)) {
                const models = companyData.models || [];
                ...
                this.elements.publicContainer.innerHTML += `<div class="company-card" data-company-name="${companyData.name}"><h3>${companyData.name}</h3><div class="model-list"><ul>${modelListHTML}</ul></div></div>`;
            }
